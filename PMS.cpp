...
void EMPLOYEE :: MODIFY_RECORD(int ecode, char ename[26], char eaddress[31], char ephone[11], char edesig[16], char egrade, char ehouse, char econv, float eloan, float ebasic)
{
	...
	fstream file ;
	file.open("EMPLOYEE.DAT", ios::out | ios::ate|ios::binary) ;
	strcpy(name,ename) ;
	strcpy(address,eaddress) ;
	strcpy(phone,ephone) ;
	strcpy(desig,edesig) ;
	...
	while (file.read((char *) this, sizeof(EMPLOYEE)))
	{
		if (code == ecode)
		{
			...
			if (grade != 'E')
			{
				...
				gotoxy(5,20) ;
				cout <<"conveyance (y/n) : " <<conveyance ;
				gotoxy(5,22) ;
...
void EMPLOYEE :: NEW_EMPLOYEE(void)
{
	...
	do
	{
		...
		if (strlen(eaddress) < 1 || strlen(eaddress) > 30)
		{
			valid = 0 ;
			gotoxy(5,25) ; clreol() ;
			cout <<"\7Enter correctly (Range: 1 to 30)" ;
			getch() ;
		}
	} while (!valid) ;
	do
	...
	do
	{
		...
		if (strlen(edesig) < 1 || strlen(edesig) > 15)
		{
			...
			gotoxy(5,25) ;
			clreol() ;
			cout <<"\7Enter correctly (Range: 1 to 15)" ;
			getch() ;
		}
	} while (!valid) ;
	do
...
void EMPLOYEE :: MODIFICATION(void)
{
	...
	if (egrade != 'E')
	{
		...
		{
			gotoxy(5,25) ; clreol() ;
			cout <<"ALLOTED CONVEYANCE ALLOWANCE or <ENTER> FOR NO CHANGE" ;
			gotoxy(22,13) ; clreol() ;
			econv = getche() ;
			econv = toupper(econv) ;
			if (econv == '0')
	...
	do
	{
		...
		if (eloan > 50000)
		{
			valid = 0 ;
			gotoxy(5,25) ; clreol() ;
			cout <<"\7SHOULD NOT BE GREATER THAN 50000" ;
			getch() ;
		}
	} while (!valid) ;
	if (strlen(t1) == 0)
...
int EMPLOYEE :: VALID_DATE(int d1, int m1, int y1)
{
	...
	if (((m1==4 || m1==6 || m1==9 || m1==11) && d1>30)||m1>12)
		valid = 0 ;
	return valid ;
}
